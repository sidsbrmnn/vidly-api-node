...
router.post('/', jwt, jwtAuthz(['write:genres']), async (req, res) => {
...
router.put('/:id', jwt, jwtAuthz(['update:genres']), async (req, res) => {
  const value = await schema.validate(req.body, { stripUnknown: true });
...
router.delete('/:id', jwt, jwtAuthz(['delete:genres']), async (req, res) => {
